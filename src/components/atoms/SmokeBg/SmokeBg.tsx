...
void main(){
  ...
  // KEY CHANGE: Instead of mixing with white (vec3(1)), we mix with our custom color.
...
class Renderer {
    // NOTE: this must be a template literal (backticks), not a plain string,
    // because it contains real line breaks. It also needs a newline after
    // "#version 300 es" -- GLSL requires the version pragma on its own line.
...
export const SmokeBackground: React.FC<AnimatedBackgroundProps> = ({
    smokeColor = "#8B0000", // Default to gray
}) => {
    ...
    return <canvas ref={canvasRef} className="w-screen fixed top-0 h-screen block" />;
};